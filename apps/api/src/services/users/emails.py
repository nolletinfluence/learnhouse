...
BESTDEVS_LANDING_URL = (
    os.environ.get("BESTDEVS_LANDING_URL")
    or os.environ.get("NEXT_PUBLIC_BESTDEVS_LANDING_URL")
    or "http://localhost:3005"
).rstrip("/")
BESTDEVS_LOGO_URL = f"{BESTDEVS_LANDING_URL}/brand/bestdevs-logo.png"
ACADEMY_URL = BESTDEVS_LANDING_URL


LOGO_SVG = (
    '<span style="display:inline-flex;align-items:center;gap:10px;color:#171717;'
    'font-size:18px;font-weight:800;">'
    '<span style="display:inline-flex;background:#0f0f10;border-radius:10px;padding:8px;">'
    f'<img src="{html.escape(BESTDEVS_LOGO_URL)}" alt="" width="28" height="28" '
    'style="display:block;width:28px;height:28px;" />'
    '</span><span>BestDevs LMS</span></span>'
)
